...
function SecurityPanel() {
  ...
  const fetchFailedTransactions = async () => {
    try {
      const res = await fetch('http://localhost:5000/api/admin/transactions/failed', {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      if (res.ok) {
  ...
  const handleToggleService = async (serviceName, enabled) => {
    try {
      const res = await fetch(`http://localhost:5000/api/admin/services/${serviceName}/toggle`, {
        method: 'POST',
        headers: {
